...
|---|---|---|
| `--pink` | `#ea4b88` | สีแบรนด์หลัก: eyebrow, ลิงก์, ตัวเลขสถิติ, hover เมนู, จุด carousel |
| `--pink-dark` | `#d83974` | hover ของลิงก์สีชมพู, outline โฟกัส |
| `--pink-soft` | `#fff2f7` | พื้นอ่อน: hover ปุ่มไอคอน, แถวตารางที่เน้น, ผลค้นหา |
...
- missions: ม่านน้ำเงิน `rgba(47,111,181,.9) → rgba(27,58,107,.93)`
- hero/stats: พื้นมืด `#1a1316` + ม่านดำโปร่ง
...
```css
linear-gradient(90deg, var(--pink) 0%, #9b6bb5 50%, var(--orange) 100%)
```

ชมพู → ม่วง → ส้ม คือ "ลายเซ็น" ของเว็บนี้ ห้ามเปลี่ยนลำดับสี
...
- **override รายเซคชัน:** ใน `.missions` ปุ่ม outline กลับเป็นขาวล้วน (พื้นน้ำเงินเข้ม) และ `.intro-btn` เป็นขาวโปร่ง + blur
...
eyebrow ชมพู uppercase → ขีดไล่สีแบรนด์ 120×5px → หัวข้อ display font จัดกึ่งกลางทั้งหมด
...
กริด `repeat(auto-fit, minmax(280px,1fr))` gap 22px · การ์ดขาว ขอบ `--border-light` โค้ง 18px · ลิงก์ในการ์ดเป็นสีชมพูขีดเส้นใต้
...
`.back-top` เป็นปุ่มกลมลอยมุมขวาล่าง โผล่เมื่อเลื่อนเกิน **500px** hover เป็นสีชมพู
...
| **Missions** | พื้นภาพตึก + ม่านน้ำเงิน · 4 การ์ดไอคอนวงกลมขาว (hover ยก 6px) · 4 → 2 → 1 คอลัมน์ |
...
| **Program blocks** | สลับซ้าย-ขวาด้วย `.reverse` · รูปสูง 420px มุมโค้ง 40px · หัวข้อสีชมพู |
| **Intro (ทำความรู้จักจุฬาฯ)** | พื้นหลังคลื่น SVG 3 ชั้นเบลอ + ดวงแสง 2 ดวง เคลื่อนช้า 22–30s · ม่านขาวไล่จากซ้ายให้ตัวอักษรอ่านง่าย · แถวสถิติสีชมพู (≤640px เป็นกริด 2×2) |
...
- **`.goal-grid` / `.goal-card`** — การ์ดเป้าหมาย มีไอคอน 46px, ท้ายการ์ดมีปุ่มกลม `.go` ที่ hover เป็นสีชมพู
- **`.staff-lead` / `.staff-grid` / `.staff-card`** — การ์ดบุคลากรรูปวงกลม มี `.initials` เป็น fallback เมื่อไม่มีรูป
- **`.table-wrap` + `.data-table`** — ตารางข้อมูล: หัวตารางเน้น, แถวคู่พื้น `#fcfcfd`, hover เป็น `--pink-soft`, แถว `.is-lead` ไฮไลต์ชมพู, `.course` จำกัดกว้าง 330px, จอเล็กเลื่อนแนวนอนได้
...
- **`.ci-list` / `.ci-item`** — แถวข้อมูลติดต่อ กริด `26px 1fr` ไอคอนเส้นสีชมพู + `.ci-title` + `.ci-text`
  ปิดท้ายด้วย `.ci-social` (ไอคอนกลมพื้น `--pink-soft` hover เป็นไล่สีชมพู→ส้ม)
...
- **`.btn-send`** — ปุ่มส่งพิลล์ไล่สีประจำแบรนด์ (ชมพู → ม่วง → ส้ม) พร้อมไอคอนเครื่องบินกระดาษ
...
- คอนทราสต์ของ `--pink` (#ea4b88) บนพื้นขาว ≈ **3.6:1** → ผ่านเกณฑ์ AA เฉพาะตัวใหญ่/ตัวหนา (≥18.5px หรือ ≥14px ตัวหนา) **ไม่ผ่านสำหรับข้อความขนาดปกติ** — ถ้าใช้กับข้อความเล็กควรเปลี่ยนเป็น `--pink-dark`
